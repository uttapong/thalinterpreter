...
exports.interprete_noMCH = function(data) {

//check for CE Machine
	//if(data.a2!=0 && data.hbe!=0){
		var a2e=data.a2+data.hbe;
		data.a2=a2e;
		data.hbe=a2e;
	//}
	//console.log('in here 1----------------------');
  if(data.f>60){
		if(data.hbe==0&&data.hbcs==0)return 'HOMO_BETA_THAL';
		if(data.hbe==0&&data.hbcs==0&&data.bart_h>=10)return 'FETUS_WITH_ALPHA_THAL';
		if(data.hbe==0&&data.hbcs==0&&data.bart_h<10)return 'NORMAL_FETAL';

		if(data.hbe>0&&data.hbcs==0)return 'FETUS_WITH_BETA_THAL_E';
		if(data.hbe>0&&data.hbcs==0&&data.bart_h>=10)return 'FETUS_WITH_HBE_ALPHA_THAL';
		if(data.hbe>0&&data.hbcs==0&&data.bart_h<10)return 'FETUS_WITH_HB_E';

		if(data.hbe>0&&data.hbcs>0&&data.bart_h>=10) return 'FETUS_WITH_HBE_CS';
	...
	if(data.a2<=3.5){
		...
		if(data.bart_h>0){
			if(data.hbcs>0){
				if(data.mcv>=80)return 'HB_HOMO_CS';
				else return 'HB_H_CS';
			}
			else if(data.hbcs<=0){
				if(data.mcv<80)return 'HB_H';
			}
			...
			if(data.hbcs>0){
				if(data.mcv>75)return 'HB_HOMO_CS';
				else return 'HB_CS';
			}
			else if(data.hbcs<=0){
				if(data.mcv>=85)return 'NORMAL_HB';
				else if(data.mcv<75)return 'ALPHA_THAL_TRAIT';
				else if(data.mcv>=75)return 'ALPHA_THAL_2';

					//if(data.mcv>=75)return 'ALPHA_THAL_2';
	...
	if(data.a2>3.5){
		if(data.a2<8&&data.mcv<75)return 'BETA_THAL';
		if(data.a2>=8){
			if(data.bart_h>0){
				if(data.hbcs>0){
					if(data.hbe>75)return 'EF_BART_CS';
					else return 'AE_BART_CS';
				}
				else if(data.hbcs<=0){
					if(data.hbe>75)return 'EF_BART';
					else return 'AE_BART';
				}
			}
			else if(data.bart_h<=0){
				if(data.hbe>35){
					if(data.hbe>75)return 'HBE_HOMO';
					else return 'BETA_THAL_HBE';
				}
				else if(data.hbe<=35){
					if(data.hbe<25||data.mcv<=70)return 'HBE_WITH_ALPHA_THAL1';
					if((data.hbe>20&&data.hbe<=35)||data.mcv>70)return 'HBE_TRAIT';
				}
			}
		}
	}
...
exports.interprete_withMCH = function(data) {
  ...
  if(data.f>60){
	...
	if(data.a2<=3.5){
		...
		if(data.bart_h>0){
			if(data.hbcs>0){
				...
				if(data.mcv<80 && data.mch<25)return 'HB_H';
			}
			...
			//console.log('in here 3----------------------');
			if(data.hbcs>0){
				if(data.mcv>75 && data.mch>20)return 'HB_HOMO_CS';
				else return 'HB_CS';
			}
			else if(data.hbcs<=0){
				if(data.mcv>=85 && data.mch>=25)return 'NORMAL_HB';
				else if(data.mcv<75 && data.mch<25)return 'ALPHA_THAL_TRAIT';
				else if(data.mcv>=75)return 'ALPHA_THAL_2';
	...
	if(data.a2>3.5){
		if(data.a2<8&&data.mcv<75 && data.mch<25)return 'BETA_THAL';
		if(data.a2>=8){
			if(data.bart_h>0){
				if(data.hbcs>0){
					if(data.hbe>75)return 'EF_BART_CS';
				...
				if(data.hbe>35){
					...
					if(data.hbe<25||(data.mcv<=70 && data.mch <25))return 'HBE_WITH_ALPHA_THAL1';
					if((data.hbe>20&&data.hbe<=35)||(data.mcv>70 && data.mch>=25))return 'HBE_TRAIT';
				}
			}
		}
	}
